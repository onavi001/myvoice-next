...
export default function RoutineAIPage() {
    ...
    useEffect(() => {
        if (!initialFlag) {
            setCurrentRoutine(routines[routines.length -1] || null);   
        }
    }, [routines]);
  
    const handleSaveRoutine = async() => {
        try {
            const routineResult = await dispatch(createRoutine({ name: currentRoutine?.name || "Nueva rutina", days: [] })).unwrap();
        ...
        <div className="min-h-screen bg-[#1A1A1A] text-white p-4">
            ...
            {currentRoutine && (
                ...
                <button
                    ...
                    className="w-full bg-[#34C759] text-black p-2 rounded-md disabled:opacity-50"
                    >
                    {loading ? "Generando..." : "Guardar Rutina"}
                    </button>
                </div>
            )}
        </div>
